...
class VL53L0X:
    def __init__(self, i2c, address=0x29, timeout_ms=_IO_TIMEOUT):
    ...
    def _spad_info(self):
        ...
        for timeout in range(self.timeout):
    ...
    def _calibrate(self, vhv_init_byte):
        ...
        self._register(_SYSRANGE_START, 0x01 | vhv_init_byte & 0xFF)

        start = utime.ticks_ms()
        while (self._register(_RESULT_INTERRUPT_STATUS) & 0x07) == 0:
            if self.timeout > 0 and \
                    (utime.ticks_diff(utime.ticks_ms(), start)) >= self.timeout:
    ...
    def set_address(self, newAddress):
        self._register(_I2C_SLAVE_DEVICE_ADDRESS, newAddress & 0x7F)
    ...
    def range(self):
        ...
        start = utime.ticks_ms()
        while (self._register(_SYSRANGE_START) & 0x01) > 0:
            if self.timeout > 0 and \
               (utime.ticks_diff(utime.ticks_ms(),start)) >= self.timeout:
                raise RuntimeError('Timeout waiting for VL53L0X!')
        ...
        range_mm = self._register(_RESULT_RANGE_STATUS + 10, struct='>H')
        self._register(_INTERRUPT_CLEAR, 0x01)
        return range_mm